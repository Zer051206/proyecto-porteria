...
import { faDownload, faTimes, faCheckCircle, faExclamationTriangle, faSignOutAlt } from '@fortawesome/free-solid-svg-icons';

export default function RecibirPaqueteForm() {
  const goBack = useGoBack();
  const navigate = useNavigate();
  const { formik, tiposPaquetes, areas, isLoading, errorCarga } = usePackagesRecibir(navigate);
    ...
    <div className="flex flex-col items-center h-screen w-screen p-4 ">
      ...
      <form onSubmit={formik.handleSubmit} className="bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-2xl mt-[70px]">
        ...
        {!isLoading && !errorCarga && (
          <fieldset className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 p-4 rounded-md border border-gray-600">
            <legend className="px-2 font-semibold text-blue-200">Datos del Paquete</legend>

            <label className="block">
              <span className="text-gray-300 text-sm font-medium">Tipo de Paquete:</span>
              <select
                name="id_tipo_paquete"
                value={formik.values.id_tipo_paquete}
                onChange={formik.handleChange}
                onBlur={formik.handleBlur}
                className="mt-1 block w-full rounded-md border-2 border-gray-600 bg-gray-700 text-gray-200 shadow-lg"
              >
        ...
        <div className="mt-6 flex justify-center w-full space-x-4">
          <button
            ...
            className="flex items-center px-6 py-2 bg-red-600 text-white font-bold rounded-md hover:bg-red-700 transition-colors duration-200"
          >
